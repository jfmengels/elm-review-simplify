...
"""
        ...
        , test "should remove branches where the condition never matches (call)" <|
            \() ->
                """module A exposing (..)
a =
  if f <| x then
...
"""
        ...
        , test "should remove branches where the condition may not match (not a && not b --> not a --> not b)" <|
            \() ->
                """module A exposing (..)
a =
  if not a && not b then
